//! `counter_demo` — a mycelium program.
//!
//! Scaffolded by `mycelium new counter_demo`. A minimal counter that initializes
...
const SEED: []const u8 = "counter_demo";
...
fn increment(ctx: mycelium.Ctx, p: *const Inc) !void {
    ...
    const sum, const carry = @addWithOverflow(s.counter, p.delta);
    if (carry != 0) return error.ArithmeticOverflow;
    s.counter = sum;
}
